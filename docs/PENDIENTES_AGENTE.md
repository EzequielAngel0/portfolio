...
`[hecho]` 2026-07-15/16 en `develop` (rama `feat/direccion-blueprint`; merge a `master` lo decide el dueño). Se implementó la FORMA blueprint (capa de plano fusionada con el circuito, geometría dura, sombra dura firma, riel lateral accesible, marquee de stack, mono amplificado) y el nombre del hero en una display condensada (Bebas Neue, subset propio de 6.4 KB, 4to woff2), SOBRE la paleta papel y tinta sin neón. Iteración 2 tras el pase visual del dueño: hero compactado, footer "estado del sistema", tema claro enfriado (`--paper` `#eef1f5`; AA re-verificado), transición de tema (View Transitions) y restyle del diagrama ACP. OG y favicons regenerados con el papel frío. `check` + `build` + Podman + reglas duras en verde. Detalle: [rondas/2026-07-15_rediseno-blueprint.md](rondas/2026-07-15_rediseno-blueprint.md); decisión y mapeo: [ADR 0011](adr/0011-direccion-stitch.md).
