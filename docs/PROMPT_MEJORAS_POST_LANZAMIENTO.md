...
`check` + `build` + `preview` en verde (o `podman compose up --build`, ya verificado que funciona), e2e rápido del flujo (tema, idioma, case study en su ruta nueva `/acp/`, página de certificaciones, CV por idioma, 404), medición de fps para T9, reglas duras por grep, y actualizar: `PENDIENTES_AGENTE.md`, el ADR 0009 y el doc del área que cambie (05/06), tracker nuevo en `docs/rondas/`, el estado de `docs/PROMPT_DESARROLLO.md` y este archivo (marcar tareas hechas). Si cambian rutas (T7) o se agregan páginas (T3), revisar sitemap/hreflang/canonical. Merge de la rama de trabajo a `develop` y push; el merge a `master` lo decide el dueño.
