...
- **Siguiente: F4 · Case study** (doc 07): páginas `/acp-suite/` y `/en/acp-suite/` (TOC sticky en desktop), SVG artesanal del diagrama por idioma (tematizado, accesible) desde el Mermaid del fuente, franja "corte de caja" con las cifras autorizadas, y carrusel accesible (GSAP Draggable) que solo aparece si hay capturas (D8). Rama `feat/...` desde `develop`.
...
Continúa el plan de `docs/07-plan-implementacion.md` **desde la fase pendiente** que marca "Estado del desarrollo" arriba (F0 a F3 ya están hechas; ahora toca **F4 · Case study**). No rehagas fases cerradas.

- **F4 (case study):** páginas `/acp-suite/` y `/en/acp-suite/` renderizando la colección `case-study` (TOC sticky en desktop, lineal en móvil); SVG artesanal del diagrama de arquitectura por idioma (tematizado con variables CSS, `role="img"` + `<title>`/`<desc>`, sin hosts/IPs/versiones) a partir del Mermaid del fuente; franja "corte de caja" con las cifras autorizadas del frontmatter; componente carrusel accesible (GSAP Draggable, degrada a scroll-snap sin JS) que solo se renderiza si hay capturas (opcionales, D8).
