...
> **Última actualización: 2026-07-16.** Un chat nuevo debe leer `docs/`, ver este estado y continuar en la fase pendiente **sin rehacer lo hecho**.
...
- **Ronda de rediseño visual "blueprint" (ADR 0011):** [hecho] 2026-07-15/16 en `develop`. Se adoptó la FORMA de la maqueta de Stitch (capturada en `DIRECCION_STITCH.md`) sobre la paleta papel y tinta, sin neón: capa blueprint de fondo fusionada con el circuito (estática, visible sin JS), hero como pieza firma (panel con sombra dura `--shadow-hard` y nombre en display condensada Bebas Neue, subset propio de 6.4 KB: 4to woff2), riel lateral de secciones accesible (CSS, `>= 80rem`), marquee de stack (CSS) y anclas nuevas en el home. Iteración 2 tras el pase visual del dueño: hero compactado, footer "estado del sistema" de 3 columnas, tema claro enfriado (`--paper` `#eef1f5`, `--surface` `#f8fafc`; AA re-verificado numéricamente), transición de tema con View Transitions y restyle del diagrama ACP (hoja de plano, aristas en acento). OG y favicons regenerados con el papel frío. `check` + `build` + Podman + reglas duras en verde. Detalle: `docs/rondas/2026-07-15_rediseno-blueprint.md`; decisión: ADR 0011.
