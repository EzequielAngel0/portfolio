...
- **F5 · SEO, assets y analítica:** [hecho] 2026-07-06 (beacon [bloqueado] por D7). `Seo.astro` completo (canonical + hreflang revisados, OG con imagen 1200×630 por página/idioma desde `src/assets/og/`, og:type article en el case study, `withAlternates` para la 404, JSON-LD: Person + SoloKey en home, SoftwareSourceCode de ACP en case study vía `src/lib/jsonld.ts`); `robots.txt` con sitemap; 404 única bilingüe (bloques ES/EN con `lang`, fuera del sitemap); favicon nuevo (monograma "A" + punto verde: `favicon.svg` tematizado, `favicon.ico` 16/32/48, `apple-touch-icon.png`). Generadores one-off en `scripts/` (satori + sharp con `--no-save`: lockfile intacta). La CSP se ajusta junto con el beacon cuando llegue D7 (doc 06). Detalle: `docs/rondas/2026-07-06_f5.md`. Rama `feat/seo-assets`.
...
- **Siguiente: F6 · Pulido y QA** (doc 07): pase `make-interfaces-feel-better`, auditorías `web-design-guidelines` y `review-animations`, Lighthouse móvil/desktop ≥ objetivos del doc 06, prueba de teclado + NVDA, validación externa de JSON-LD (Rich Results) y OG (previewer), `frontend-code-review` + `/code-review` del diff, verificación end-to-end con `preview`. Rama `feat/...` desde `develop`. El punto del beacon de F5 se retoma cuando el dueño resuelva D7.
